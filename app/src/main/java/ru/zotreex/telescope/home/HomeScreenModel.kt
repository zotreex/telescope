...
class HomeScreenModel(private val tdClient: TdClient) : ScreenModel {
        ...
        screenModelScope.launch {
            ...
            val newList = chatIds.map {
                async {
                    ...
                    val test = if(list.size == 1) getMessagesByChatId(it, startId = list[0].id) else emptyList()
                    list + test
                }
            }.awaitAll()
    ...
    private suspend fun filterMessage(content: TdApi.MessageContent, message: TdApi.Message): VideoContent {
        if (content is TdApi.MessageVideo) {
            return VideoContent(content.caption.text.toString(), messageId = message, downloadFile(content.video.thumbnail?.file))
        } else {
            (content as MessageVideoNote).let {
                return VideoContent(content.videoNote.video.remote.id, messageId = message, downloadFile(content.videoNote.thumbnail?.file))
            }
        }
    }
